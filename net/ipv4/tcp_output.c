				      ...
				      u64 prior_wstamp)
{
	...
	if (sk->sk_pacing_status != SK_PACING_NONE) {
		unsigned long rate = td_pacing_rate(sk);
...
static void tcp_adjust_pcount(struct sock *sk, const struct sk_buff *skb, int decr)
{
	...
	if (tcp_is_reno(tp) && decr > 0)
		td_set_sacked_out(tp, skb_tdn, td_get_sacked_out(tp, skb_tdn) -
			       min_t(u32, td_get_sacked_out(tp, skb_tdn), decr));
			    ...
			    int min_tso_segs)
{
	...
	bytes = min_t(unsigned long,
		      td_pacing_rate(sk) >> READ_ONCE(sk->sk_pacing_shift),
				  ...
				  unsigned int factor)
{
	...
	if (static_branch_unlikely(&tcp_tx_delay_enabled) &&
		...
		u64 extra_bytes = (u64)td_pacing_rate(sk) * tcp_sk(sk)->tcp_tx_delay;
